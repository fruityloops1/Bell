...
PATCH_NSO_MAP_DIR = os.path.join(PATCH_DIR, "syms")
PATCH_CONFIG_EXTENSION = ".config"

IPS_OUT_DIR_NAME = "build{}"
IPS_FORMAT = ".ips"
...
def getSymAddrFromMap(target, regexStr, symStr):
    ...
    def getFoundPosAddr(start):
        ...
        foundAddr = int(foundLine.replace(' ', '').split('=')[1].replace(';', ''), 0)
...
def resolveAddressAndTarget(target, symbolStr):
    ...
    try:
        resolvedAddr += getSymAddrFromMap('donotusethistargetunderanycircumstance', regexStr, symbolStr)
    except NameError:
...
initConfig()
SLMapFilePath = os.path.join("build" + buildVersion, os.path.basename(os.getcwd()) + buildVersion + ".sym")
SLMapFile = ""
with open(SLMapFilePath, 'r') as f:
    SLMapData = str(f.read())
    for line in SLMapData[SLMapData.find('SYMBOL TABLE:'):].replace('SYMBOL TABLE:\n', '').split('\n'):
        rows = line.split()
        if len(rows) >= 6 and not rows[2].startswith('.') and rows[2] == 'F':
            addr = int('0x' + rows[0], 0)
            symbol = rows[5]
            SLMapFile += symbol + ' = 0x{:08x};\n'.format(addr)
